...
class AutenticarUsuarioUseCase:
    ...
    def __init__(self, auth_service: IAuthService, jwt_service: IJWTService):
    ...
    async def execute(self, request: LoginDTO) -> Optional[TokenDTO]:
        ...
        try:
            ...
            if not user_data:
                logger.warning("Tentativa de login inválida", username=request.username)
            ...
            refresh_token = self.jwt_service.create_refresh_token(
                refresh_token_data, expires_delta=timedelta(days=7)
            )
