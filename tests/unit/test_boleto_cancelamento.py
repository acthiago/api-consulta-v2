...
class TestBoletoCancelamento:
    ...
    def test_status_restauracao_divida(self):
        ...
        status = ("ativo" if dias_atraso <= 0 else
                  ("vencido" if dias_atraso <= 30 else "inadimplente"))
        assert status == "ativo"
        ...
        status = ("ativo" if dias_atraso <= 0 else
                  ("vencido" if dias_atraso <= 30 else "inadimplente"))
        # Pode variar dependendo da data atual
        assert status in ["vencido", "inadimplente"]
    ...
    def test_validacao_status_boleto(self):
        ...
        for status in status_validos:
            assert status not in ["pago", "cancelado"], (
                f"Status {status} deveria permitir cancelamento"
            )

        for status in status_invalidos:
            assert status in ["pago", "cancelado"], (
                f"Status {status} não deveria permitir cancelamento"
            )
    ...
    def test_estrutura_resposta_cancelamento(self):
        ...
        resposta_esperada = {
            ...
            "dividas_restauradas": [
                "68ae745b62c4c9bfff79f166",
                "68ae745b62c4c9bfff79f167"
            ],
            "historico_preservado": True,
            "message": ("Boleto cancelado com sucesso! "
                        "2 dívida(s) restaurada(s) ao estado original.")
        }
        ...
        for campo in campos_obrigatorios:
            assert campo in resposta_esperada, (
                f"Campo {campo} é obrigatório na resposta"
            )
